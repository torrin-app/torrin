...
func TestBachsCreateSubscriptionCheckout(t *testing.T) {
	var got map[string]any
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		json.NewDecoder(r.Body).Decode(&got)
		json.NewEncoder(w).Encode(map[string]any{"checkout_url": "https://checkout.bachs.io/c/sub1"})
	}))
	defer srv.Close()

	b := NewBachsHandler(srv.URL, "sk", "whsec", "prod_1", "standard-monthly:prod_sub", "", "https://torrin.app", "", nil)
	url, err := b.CreateCheckout(context.Background(), "a@b.com", "standard", "monthly", 0)
	if err != nil {
		t.Fatal(err)
	}
	if url != "https://checkout.bachs.io/c/sub1" {
		t.Errorf("checkout_url = %q", url)
	}
	item := got["product_cart"].([]any)[0].(map[string]any)
	if item["product_id"] != "prod_sub" {
		t.Errorf("recurring checkout should use the sub product, got %v", item)
	}
	if _, hasAmount := item["amount"]; hasAmount {
		t.Errorf("recurring checkout must not send an amount, got %v", item)
	}
}
